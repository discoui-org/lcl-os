...
def is_android_core_stale(android_core: Path) -> bool:
    ...
    for d in (
        ROOT_DIR / "platforms" / "android",
        ROOT_DIR / "platforms" / "common",
        ROOT_DIR / "system" / "gpu",
        ROOT_DIR / "system" / "ipc",
        ROOT_DIR / "system" / "render",
        ROOT_DIR / "frameworks" / "graphics",
    ):
        if d.is_dir():
            for p in d.rglob("*"):
...
def is_android_images_stale(out_system_img: Path, out_ramdisk_img: Path,
                            android_core: Path, rasterd: Path, gpud: Path) -> bool:
    ...
    if rasterd.is_file() and rasterd.stat().st_mtime > sys_mtime:
        return True
    if gpud.is_file() and gpud.stat().st_mtime > sys_mtime:
        return True
    scripts = [ROOT_DIR / "tooling" / "build" / "build_android_images.py"]
    for s in scripts:
...
def build_targets(env: AndroidEnvironment, force_rebuild: bool = False, arch: str = "x86_64") -> None:
    ...
    if force_rebuild or is_android_core_stale(android_core) or not rasterd.is_file() or not gpud.is_file():
        log("Building Android compositor, raster service and GPU broker...")
        if not env.ndk_root:
        ...
        subprocess.run([
            ...
            "--target", "lcl-core-android", "lcl-rasterd-android", "lcl-gpud-android",
...
def launch_avd(args: argparse.Namespace) -> None:
    ...
    out_system_img = ANDROID_IMAGES_DIR / "lcl-system.img"
    out_ramdisk_img = ANDROID_IMAGES_DIR / "lcl-ramdisk.img"
    android_core = BUILD_ANDROID_DIR / "lcl-core-android"
    rasterd = BUILD_ANDROID_DIR / "lcl-rasterd-android"
    gpud = BUILD_ANDROID_DIR / "lcl-gpud-android"
    ...
    if not args.no_build:
        ...
        if args.rebuild or is_android_images_stale(
                out_system_img, out_ramdisk_img, android_core, rasterd, gpud):
            from tooling.build.build_android_images import build_android_images
