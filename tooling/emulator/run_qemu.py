...
from tooling.paths import ISO_IMAGES_DIR, QEMU_CACHE_DIR, ROOTFS_DIR, qemu_build_dir

BUILD_DIR = qemu_build_dir("x86_64")
...
DOCKERFILE = ROOT_DIR / "tooling" / "build" / "Dockerfile.qemu"
...
def configure_output_paths(arch: str) -> None:
    """Select the architecture-specific QEMU output tree for this invocation."""
    global BUILD_DIR, INITRAMFS_DIR, INITRAMFS_IMG
    BUILD_DIR = qemu_build_dir(normalize_arch(arch))
    INITRAMFS_DIR = BUILD_DIR / "initramfs_root"
    INITRAMFS_IMG = BUILD_DIR / "initramfs.cpio.gz"
...
def ensure_fonts() -> None:
    ...
    if not inter.is_dir() or not any(inter.iterdir()):
        fetch = ROOT_DIR / "tooling" / "build" / "fetch_fonts.py"
        if fetch.is_file():
...
def locate_kernel() -> Path:
    ...
    err("No kernel in Docker image. Rebuild using tooling/build/Dockerfile.qemu")
    sys.exit(1)
    ...
    if need_build:
        ...
        try:
            ...
            log(
                ...
                "Fix ownership under out/cache if needed. "
                "The Docker image was built successfully and QEMU will proceed."
            )


def fix_permissions() -> None:
    """Fix ownership of the selected out/qemu tree after a Docker build."""
...
def clean_build(arch: str = "x86_64") -> None:
    ...
    if not BUILD_DIR.exists():
        log(f"{BUILD_DIR} does not exist. Nothing to clean.")
        return
    log(f"Cleaning {BUILD_DIR}...")
    try:
        shutil.rmtree(BUILD_DIR)
        log(f"Successfully removed {BUILD_DIR}")
    except Exception:
        if docker_available():
            log(f"Removing {BUILD_DIR} via Docker container...")
            mount = str(ROOT_DIR)
            container_build = f"/src/{BUILD_DIR.relative_to(ROOT_DIR).as_posix()}"
            run(["docker", "run", "--rm", "-v", f"{mount}:/src", docker_image_name(arch), "rm", "-rf", container_build])
            log(f"Successfully removed {BUILD_DIR} via Docker")
        else:
            err(f"Failed to remove {BUILD_DIR}; fix its ownership and retry.")
            sys.exit(1)
...
def docker_build_and_package(diagnostic_args: list[str], arch: str = "x86_64") -> Path:
    ...
    if not docker_available():
        ...
        err(f"Install Docker, then: docker build --platform {docker_platform(arch)} -f tooling/build/Dockerfile.qemu -t {docker_image_name(arch)} tooling/build/")
        sys.exit(1)
...
def package_inside_docker(args: argparse.Namespace) -> Path:
    ...
    from tooling.build.build_rootfs import build_rootfs_ext4
    build_rootfs_ext4(
        arch=args.arch,
        inside_docker=True,
        force=args.rebuild,
...
def validate_kernel(kernel: Path) -> None:
    ...
    if not os.access(kernel, os.R_OK):
        err(f"Kernel not readable (fix ownership under out/cache/qemu): {kernel}")
        sys.exit(1)
    try:
...
def launch_qemu(
    ...
    if iso_mode:
        ...
        iso_file = ISO_IMAGES_DIR / iso_name if (ISO_IMAGES_DIR / iso_name).is_file() else ISO_IMAGES_DIR / "lcl-os.iso"
    ...
    if iso_mode:
        iso_name = f"lcl-os-{arch}.iso"
        iso_path = ISO_IMAGES_DIR / iso_name if (ISO_IMAGES_DIR / iso_name).is_file() else ISO_IMAGES_DIR / "lcl-os.iso"
        if not iso_path.is_file():
            err(f"Missing ISO file: {iso_path}. Please run 'make iso ARCH={arch}' first.")
            sys.exit(1)
        cmd.extend(["-boot", "d", "-cdrom", str(iso_path)])
    else:
        rootfs_img = ROOTFS_DIR / f"lcl-rootfs-{arch}.ext4"
...
def main() -> None:
    ...
    parser.add_argument("--iso", action="store_true", help="Boot from out/images/iso/lcl-os.iso")
    parser.add_argument("--uefi", action="store_true", help="Use OVMF UEFI firmware for QEMU boot")
    parser.add_argument("--clean", action="store_true", help="Remove the selected out/qemu/<arch> tree")
    parser.add_argument(
        "--arch",
        "-a",
        metavar="ARCH",
