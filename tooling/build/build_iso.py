...
from tooling.paths import ISO_IMAGES_DIR, QEMU_CACHE_DIR, qemu_build_dir

ISO_ROOT = ISO_IMAGES_DIR / "staging"
...
def build_iso(arch: str = "x86_64") -> Path:
    ...
    output_iso = ISO_IMAGES_DIR / f"lcl-os-{target_arch}.iso"
    generic_iso = ISO_IMAGES_DIR / "lcl-os.iso"
    ...
    run_qemu = PROJECT_ROOT / "tooling" / "emulator" / "run_qemu.py"
    subprocess.check_call([sys.executable, str(run_qemu), "--package-only", "--arch", target_arch])

    kernel_src = QEMU_CACHE_DIR / "vmlinuz"
    initramfs_src = qemu_build_dir(target_arch) / "initramfs.cpio.gz"
    ...
    # Copy limine.conf
    limine_conf_src = PROJECT_ROOT / "packaging" / "iso" / "boot" / "limine.conf"
