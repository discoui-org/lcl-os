...
public:
  ...
  std::vector<DockWindow> items() const {
    ...
    for (const auto &scene : m_state.snapshot().scenes) {
      if (scene.visibility == lcl::protocol::LCLSceneVisibility::Closing)
        continue;
...
iconCatalog(const std::vector<lcl::session::CatalogEntry> &entries) {
  std::unordered_map<std::string, std::string> result;
  for (const auto &app : entries) {
    if (app.appId.empty() || app.icon.empty())
      continue;
...
DockLayout layoutDock(uint32_t width, uint32_t height, size_t windows) {
  DockLayout out{};
  out.height = std::min<int>(
      static_cast<int>(height - std::min(height, kDockBottomInset)),
      kDockHeight - kDockBottomInset);
  out.y = std::max(0, static_cast<int>(height) - out.height -
                          static_cast<int>(kDockBottomInset));
  if (windows == 0 || out.height <= 0)
    return out;
  const int maxIcons =
      std::max(1, (static_cast<int>(width) - kIconPadX * 2 + kIconGap) /
                      (kIconSize + kIconGap));
  out.count = std::min<int>(static_cast<int>(windows), maxIcons);
  out.first = std::max(0, static_cast<int>(windows) - out.count);
  out.width = out.count * kIconSize + std::max(0, out.count - 1) * kIconGap +
              kIconPadX * 2;
...
std::unique_ptr<lcl::ui::Container> makeDockView(DockView &view, uint32_t width,
                                                 uint32_t height) {
  view = {};
  view.width = width;
  view.height = height;
  ...
  panel->setBorderColor({255, 255, 255, 68});
  panel->setBorderWidth(1.0f);
  panel->setBorderRadius(26.0f);
  panel->setBorderRoundness(2.0f);
  ...
  backdrop->addFilter(lcl::protocol::FilterType::Glass, 20.0f, 5.0f, 0.0f);
  panel->addChild(std::move(backdrop));

  auto innerBorder = std::make_unique<lcl::ui::Container>();
  view.innerBorder = innerBorder.get();
  innerBorder->setBorderColor({0, 0, 0, 34});
  innerBorder->setBorderWidth(1.0f);
  innerBorder->setBorderRadius(24.0f);
  innerBorder->setBorderRoundness(2.0f);
...
void updateDockView(
    DockView &view, const std::vector<DockWindow> &windows,
    const std::unordered_map<std::string, std::string> &catalog) {
  if (!view.root || !view.panel || !view.backdrop || !view.innerBorder)
    return;
  for (auto *child : view.dynamicChildren)
    view.panel->removeChild(child);
  ...
  absolute(*view.innerBorder, 2, 2, std::max(0, layout.width - 4),
           std::max(0, layout.height - 4));
  ...
  for (int i = 0; i < layout.count; ++i) {
    ...
    if (window.focused) {
      ...
      indicator->setBackgroundColor({246, 248, 252, 238});
      indicator->setBorderRadius(1.5f);
      absolute(*indicator, iconX + kIconSize / 2 - 10, iconY + kIconSize + 4,
               20, 3);
...
int main() {
  ...
  if (session.connect() &&
      !session.requestCatalog(catalogEntries, catalogError)) {
  ...
  auto createPanels = [&]() -> bool {
    ...
    dock->setInitialBounds(
        0,
        static_cast<int32_t>(height > kDockHeight ? height - kDockHeight : 0),
        width, kDockHeight);
    ...
    if (!dock->connectCompositor())
      return false;
    if (!menu->connectCompositor())
      return false;
