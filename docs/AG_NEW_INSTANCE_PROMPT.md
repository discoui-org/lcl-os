...
```
Example:
`AntiGravity: Sayın Gemini ve Sayın User, ...`
...
6. **Terminal & Rendering Subsystem:**
   - Interactive shell powered by GNU Bash 5.3 + readline (`menu-complete`).
   - Font-agnostic canvas layout (`FontRenderer::getCellWidth()`, `getCellHeight()`) and dynamic PTY winsize (`TIOCSWINSZ`) synchronization.
   - VT100 Write-Head overwrite model for accurate PTY line buffer tracking.
   - Typing-aware 500ms blinking inverted block cursor.
