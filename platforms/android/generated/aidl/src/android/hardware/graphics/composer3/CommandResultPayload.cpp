...
binder_status_t CommandResultPayload::readFromParcel(const AParcel* _parcel) {
  ...
  case clientTargetProperty: {
    ...
    return STATUS_OK; }
  case displayLuts: {
    ::aidl::android::hardware::graphics::composer3::DisplayLuts _aidl_value;
    if ((_aidl_ret_status = ::ndk::AParcel_readData(_parcel, &_aidl_value)) != STATUS_OK) return _aidl_ret_status;
    if constexpr (std::is_trivially_copyable_v<::aidl::android::hardware::graphics::composer3::DisplayLuts>) {
      set<displayLuts>(_aidl_value);
    } else {
      // NOLINTNEXTLINE(performance-move-const-arg)
      set<displayLuts>(std::move(_aidl_value));
    }
    return STATUS_OK; }
  }
...
binder_status_t CommandResultPayload::writeToParcel(AParcel* _parcel) const {
  ...
  case displayLuts: return ::ndk::AParcel_writeData(_parcel, get<displayLuts>());
  }
