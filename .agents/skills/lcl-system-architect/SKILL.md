...
- Keep ScrollContent raster storage in rasterd as bounded 384-logical-pixel vertical tiles. Retain the initial logical cache body and damage patches so a property-only scroll can create an entering tile without client DisplayList/image upload. Hold only viewport plus one-tile overscan residency, evict distant/removed tiles after successful immutable publication, and never advance cache namespaces on a failed transaction.
- Cache small bounded non-scroll transform/opacity subtrees in identity-space rasterd layers. Reuse the texture for property-only translation, scale, rotation, and opacity commits; collapse nested candidates, exclude ScrollView-containing subtrees, and release namespaces only after a successful immutable output commit.
