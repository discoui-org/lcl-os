...
- **Unix Domain Socket Backend:** Compositor IPC uses Unix Domain
  `SOCK_SEQPACKET` at `/run/user/1000/lcl-compositor.sock`. Protocol v3 keeps
  each explicit little-endian header, payload, and optional `SCM_RIGHTS`
  descriptor in one packet; stream and text-command fallbacks are forbidden.
